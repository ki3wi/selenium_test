...
class ParseExcel(object):
    ...
    def getCellOfObject(self,sheet,coordinate=None,rowNo=None,colsNo=None):
        ...
        #如getCellObject(sheet,coordinate='A1')or getCellObject(sheet,rowNo=1,cols=2)
        if coordinate!=None:
            try:
    ...
    def writeCell(self,sheet,content,coordinate=None,rowNo = None,colsNo=None,style=None):
        #根据单元格在excel中的编码坐标或者狮子索引坐标像单元格中写入数据，
        ...
        if coordinate is not None:
            ...
            try:
                ...
                self.workbook.save(self.excelFile)
                #print 'coordinate is :%s' %coordinate
            except Exception,e:
                raise
        else:
...
if __name__ == '__main__':
    pe = ParseExcel()
    pe.loadWorkBook(u'C:\\Users\\ki3wi\\PycharmProjects\\selenium_test\\DataDrivenFrameWork\\testData\\126邮箱联系人.xlsx')
    print u"通过名称获取sheet对象名字：",pe.getSheetByName(u'联系人').title
    print u"通过index序号获取sheet对象的名字：",pe.getSheetByIndex(0).title
    sheet = pe.getSheetByIndex(0)
    print type(sheet)
    print u"最大行数：%d" % pe.getRowNumber(sheet)# 获得最大行数
    print u"最大列数：%d" % pe.getColsNumber(sheet)# 获得最大列号
    # 获取第一行
    ...
    pe.writeCell(sheet,u'我爱祖国',rowNo=10,colsNo=10,style='red')
